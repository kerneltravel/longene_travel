        ...
        unsigned BufLen, LPWSTR BuiltinCmdLine, LPWSTR BuiltinAppName, unsigned BuiltinAppNameLen)
{
    ...
    if (L'"' == CmdLine[0] && (p = strchrW(CmdLine + 1, L'"'))) {
        ...
        Name = RtlAllocateHeap(GetProcessHeap(), 0, max(((Len + 1) * sizeof(WCHAR)), strlenW(p)));
        if (!Name) {
        ...
        LPPROCESS_INFORMATION lpProcessInformation)
{
	...
	ppb->hStdInput = INVALID_HANDLE_VALUE;
	ppb->hStdOutput = INVALID_HANDLE_VALUE;
