...
static unsigned long find_symbol_in_file(struct elf_phdr *elf_phdata, int phnum, 
	elf_shdr *elf_shdata, int shnum, char *sym, unsigned long base_addr)
{
	int sym_count = 0;
	int		link = -1;
	int		str_tab_size = 0;
...
int check_exeso(struct linux_binprm * bprm)
{
	...
	elf_shdata = (elf_shdr *)(map_addr + elf_ex->e_shoff);
...
static int load_exeso_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
	...
	/* set the teb */
	init_teb.StackBase = (PVOID)stack_top;
	init_teb.StackLimit = (PVOID)stack_top - WIN32_STACK_LIMIT;
