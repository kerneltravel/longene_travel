...
void trapframe_to_context(struct ethread * thread, PCONTEXT context)
{
	...
	if ((context->ContextFlags & CONTEXT_SEGMENTS) == CONTEXT_SEGMENTS) {
		...
		context->SegFs = thread->et_task->thread.fs;
		context->SegGs = thread->et_task->thread.gs;
	}

	if ((context->ContextFlags & CONTEXT_DEBUG_REGISTERS) == CONTEXT_DEBUG_REGISTERS) {
		context->Dr0 = thread->et_task->thread.debugreg0;
		context->Dr1 = thread->et_task->thread.debugreg1;
		context->Dr2 = thread->et_task->thread.debugreg2;
		context->Dr3 = thread->et_task->thread.debugreg3;
		context->Dr6 = thread->et_task->thread.debugreg6;
		context->Dr7 = thread->et_task->thread.debugreg7;
	}
}
...
void context_to_trapframe(PCONTEXT context , struct ethread * thread)
{
	...
	if ((context->ContextFlags & CONTEXT_SEGMENTS) == CONTEXT_SEGMENTS) {
		...
		thread->et_task->thread.fs = context->SegFs;
		thread->et_task->thread.gs = context->SegGs;
	}	

	if ((context->ContextFlags & CONTEXT_DEBUG_REGISTERS) == CONTEXT_DEBUG_REGISTERS) {
		thread->et_task->thread.debugreg0 = context->Dr0;
		thread->et_task->thread.debugreg1 = context->Dr1;
		thread->et_task->thread.debugreg2 = context->Dr2;
		thread->et_task->thread.debugreg3 = context->Dr3;
		thread->et_task->thread.debugreg6 = context->Dr6;
		thread->et_task->thread.debugreg7 = context->Dr7;
	}
