...
void fd_assign_completion(struct fd *fd, struct uk_completion **p_port, unsigned long *p_key)
{
	*p_key = fd->comp_key;
	*p_port = fd->completion ? (struct uk_completion *)grab_object(fd->completion) : NULL;
}
